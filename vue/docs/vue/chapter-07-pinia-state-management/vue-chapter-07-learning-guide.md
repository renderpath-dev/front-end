...
本章定义 Pinia 作为 global client state owner 的边界。`stores/pinia.ts` 创建唯一 Pinia root 并注册 `piniaPersistencePlugin.ts`；`authStore.ts`、`permissionStore.ts`、`themeStore.ts`、`preferenceStore.ts`、`cartStore.ts`、`selectionStore.ts`、`sidebarStore.ts` 分别展示 Option Store、Setup Store、derived getters、actions、persistence 和 collection state；Chapter 06 的 `authSession.ts`、`authGuard.ts`、`permissionGuard.ts`、`dynamicMenu.ts` 被改造成读取 Pinia 的 adapter；`StoreToRefsDemo.vue`、`PatchResetDemo.vue`、`PiniaPluginDemo.vue`、`StoreOutsideComponentDemo.vue`、`SsrSafeStateDemo.vue` 和 `VuePiniaAdminStateLab.vue` 提供可观察 evidence。

执行 owner 是 Pinia root + Vue reactivity。`createPinia()` 安装到 app 后，`defineStore` 返回的 use function 在当前 active pinia 中创建 store instance；state factory 产生 reactive state，getters 类似 computed，actions 封装 mutation / async flow，plugin 在 store creation 时扩展 store 并通过 `$subscribe` 观察 mutation。TypeScript 能推断 state/getter/action shape、store id 和 plugin payload 的静态结构，但不能验证 localStorage JSON 是否可信，不能保证用户真的有权限，也不能自动隔离 SSR request 中的 module-global reactive object。

跨边界的值包括 store id、state factory return object、getter return、action payload、`storeToRefs` 生成的 refs、`$patch` object/function、persisted envelope、Pinia plugin context、Router guard 中显式传入的 pinia root。它纠正的误解是“所有跨组件 state 都该进 store”或“Pinia state 就等于后端事实”。本章不接管 URL query、component-only draft、server cache、backend authorization 或 API schema；这些分别属于 Router、component/composable、future cache/server、backend 和 Chapter 09。
...
| Store path | Concrete owner | What changes | Failure signal |
| --- | --- | --- | --- |
| `stores/pinia.ts` | one Pinia root installed before Router | plugins attach before stores are used | guard import store before active pinia causing instance/order bug |
| `authStore.ts` | Option Store owns user/session role | `signInDemoUser` mutates declared state and getters recompute | duplicate auth state remains in Chapter 06 module local adapter |
| `permissionStore.ts` | Setup Store composes auth store | `canAccessRouteMeta` reads route meta + current role | menu and guard disagree because permission logic duplicated |
| `cartStore.ts` | collection state + getters/actions | `$patch` groups item changes, totals derive from rows | derived totals stored separately and drift after mutation |
| `StoreToRefsDemo.vue` | component reads store reactive wrapper | `storeToRefs` keeps state/getter connection, actions stay bound | direct destructuring freezes primitive state snapshot |
| `piniaPersistencePlugin.ts` | plugin handles allowlisted localStorage envelope | `$subscribe` serializes safe stores, restore validates version/storeId | token or arbitrary JSON restored as trusted state |
| `StoreOutsideComponentDemo.vue` | guards/plain modules call store at execution time | explicit `pinia` avoids ambiguous app context | top-level `useStore()` runs before app installs Pinia |
| `SsrSafeStateDemo.vue` | per-app/request store instance | state factory prevents cross-user sharing | module-global reactive object leaks user state in SSR model |
| Router adapters | Chapter 06 guards/menu read Pinia stores | URL/meta remain Router-owned; auth/permission client state becomes Pinia-owned | treating Pinia permission as backend authorization |
| `pinia-lab/VuePiniaAdminStateLab.vue` | final lab coordinates auth/theme/cart/sidebar/selection | single root state powers multiple panels without second app | every small UI toggle becomes global state |
...
以下code windows给出core stores、local plugin、Chapter06 adapter、关键component与final lab的完整代码。样式与其余focused panels保留在真实文件中，本章代码定位索引用于确认路径边界。
...
| State type | Put in Pinia when | Keep elsewhere when | Evidence | Future / external owner |
| --- | --- | --- | --- | --- |
| Auth/theme/preferences | Multiple routes/components need the same client state and safe persistence | It is only a local button/modal state | reload restores allowlisted non-secret state; guards/menu read same store | Backend owns real auth/session authority |
| Cart/selection/sidebar | Multiple admin panels coordinate collection or layout state | Selection belongs to one table instance | action trace and getters explain every mutation | Component state for local-only selection |
| Router-derived filters | Pinia may read them but should not own shareable URL state | filter/page/sort must survive copy URL and back/forward | URL query is source, store does not duplicate it | Chapter 06 Router |
| Server data | Store only small client projection if no server cache policy exists | data freshness, invalidation, pagination, retry matter | stale policy is explicit, not accidental state reuse | Chapter 09 / future server cache layer |
| Persistence | Only versioned, allowlisted, non-secret state | token, role proof, private profile, arbitrary JSON | parser rejects bad envelope and unknown keys | Server session / secure cookie / backend |
| Store API size | actions express domain mutation and getters are pure | store becomes dumping ground for every composable | tests create fresh Pinia and assert action/getter result | Chapter 10 tests catch hidden coupling |

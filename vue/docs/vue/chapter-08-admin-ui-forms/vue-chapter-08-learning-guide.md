...
本指南位于 `docs/vue/chapter-08-admin-ui-forms/vue-chapter-08-learning-guide.md`，可运行代码位于 `src/learning/vue/chapter-08-admin-ui-forms/`。应用仍由 Chapter 01 的 `main.ts` 创建唯一 app；Chapter 08 只注册 plugin、读取既有 Router/Pinia，并嵌入既有 `/` 学习页。
...
先运行 dashboard，再观察 9.1–9.6 的层级与 query；接着完成 9.7–9.11 的 draft、validation、CRUD、upload；然后学习 9.12–9.16 的权限、theme、i18n 和分层；最后用 9.17–9.19 重建整条集成链。
...
- Guide：`docs/vue/chapter-08-admin-ui-forms/vue-chapter-08-learning-guide.md`
- Chapter entry：`src/learning/vue/chapter-08-admin-ui-forms/AdminUiChapterApp.vue`
- UI/i18n：`ui/*.ts`、`i18n/*.ts`
- Contracts：`contracts/adminUiTypes.ts`、`adminMockData.ts`、`tableContracts.ts`、`formContracts.ts`、`uploadContracts.ts`
- Composables：`useAdminTableQuery.ts`、`useAdminPagination.ts`、`useDialogFormState.ts`、`useDrawerFormState.ts`、`useLocalCrudCollection.ts`、`useUploadQueue.ts`、`useButtonPermission.ts`、`useElementFormValidation.ts`、`useAdminTabs.ts`
- Layout：`AdminLayout.vue`、`AdminSidebar.vue`、`AdminTopbar.vue`、`AdminBreadcrumb.vue`、`AdminTabs.vue`
- Forms：`SearchForm.vue`、`UserForm.vue`、`ProductForm.vue`、`RoleForm.vue`、`OrderStatusForm.vue`、`EditDialog.vue`、`EditDrawer.vue`
- Tables：`DataTablePage.vue`、`UsersTablePage.vue`、`ProductsTablePage.vue`、`OrdersTablePage.vue`、`RolesTablePage.vue`
- Upload/permissions/theme：`upload/*.vue`、`permissions/*.vue`、`theme/*.vue`
- Final dashboard：`admin-dashboard/VueAdminDashboard.vue` 及七个 focused child components
- Existing integration：Chapter 01 `main.ts` 与 `App.vue`
- Tooling/dependencies：`package.json`、`package-lock.json`、`tsconfig.app.json`

...
- 只用于整合前文机制的最终小项目。
- 额外速查表。
- 本章机制边界、核心机制证据链总览和真实项目判断模型。
- 官方文档阅读清单。

真实文件路径必须在本地存在，并在目录结构、代码定位索引和代码窗口标题栏中一致。概念示例使用 `Snippet:` 或 `Template:`，不得伪装成真实文件。

文件位置、更新文件和交付文件列表属于最终响应校验内容，不作为长期学习指南正文的主体章节保存。

`本章机制边界`、`核心机制证据链总览` 和 `真实项目判断模型` 不得使用可复用样板。它们必须从当前章节的 `## 本章代码定位索引`、实际 `9.x` headings、命名文件、命名 API、组件、route、store、config、command 和最终小项目机制中推导。如果只替换主题名就能复制到另一章，视为 `FAIL`。

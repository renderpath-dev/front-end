...
`## 0. 本章机制边界`、`## 4. 核心机制证据链总览` 和 `## 14. 真实项目判断模型` 必须从当前章节的实际 `9.x` headings、代码定位索引、命名文件、命名 API、组件、route、store、config、command 和最终小项目机制中推导。不得使用跨章节复用的四句 skeleton 或同一张泛化表。若只替换章节名或主题名就能复制到另一章，更新失败。
...
新章节的目录名、文件名、H1、目录、代码定位索引和代码窗口路径应保持同一章节身份。更新已有章节时，不为满足格式而无理由改名。
...
- `## 目录` 位于 H1 和 style block 之后、`## 0. 本章机制边界` 之前。
...
- 代码定位索引是 concept-to-code evidence index，用于证明某个机制在哪个真实练习或片段中可观察，不是交付文件列表。
- 索引中的真实路径必须与推荐结构、代码窗口标题栏和最终项目结构一致。
...
`## 14. 真实项目判断模型` 用于帮助学习者把章节机制迁移到真实项目决策中。它必须说明：

- 什么时候应该使用本章技术。
- 什么时候不应该使用，或应退回更简单方案。
- 关键 trade-off、risk signal 和 failure recognition。
- 真实项目中需要哪些 evidence 才能确认方案可维护、可测试、可发布。
- 哪个未来章节、后端、server runtime、deployment layer、test layer 或外部平台拥有被排除的 concern。

创建或更新的文件列表只放在最终响应的交付证据中，不放入长期学习指南正文。

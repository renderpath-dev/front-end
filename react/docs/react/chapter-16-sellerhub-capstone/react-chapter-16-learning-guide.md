...
| --- | --- | --- | --- | --- |
| Feature scope | The capstone owns a complete but bounded product slice. | Product engineering boundary | SellerHub combines catalog, cart, checkout, orders, operations, and evidence pages. | `src/learning/react/chapter-16-sellerhub-capstone/01-capstone-scope/capstone-scope-panel.tsx` |
| Route state and shell | The app router owns page composition and URL-driven state. | React Router runtime | Capstone navigation keeps feature pages reachable and reviewable. | `src/learning/react/chapter-16-sellerhub-capstone/sellerhub-capstone-app/app/sellerhub-router.tsx` |
| DTO adapter and reducer | External data conversion and local business transitions stay separate. | Client API plus React state architecture | Catalog, cart, checkout, and orders use explicit boundaries. | `src/learning/react/chapter-16-sellerhub-capstone/sellerhub-capstone-app/shared/api/sellerhub-adapters.ts` |
| Portfolio evidence | Documentation and tests prove behavior instead of claiming completeness. | Testing and documentation boundary | Release checklist, ADR, tests, and portfolio evidence make the capstone auditable. | `src/learning/react/chapter-16-sellerhub-capstone/sellerhub-capstone-app/docs/portfolio-evidence.md` |

## 0. 本章工程问题与边界

本章解决的工程问题是：把前面章节的机制组合成一个可审查的 SellerHub 功能切片。capstone 的价值不在“代码多”，而在 route、DTO、state、forms、permissions、observability、tests 和 documentation 的边界能被解释和验证。

本章不声称是完整生产系统，不新增真实支付、真实认证、真实后端或部署流水线。边界是学习项目中的 production-style feature delivery evidence。
...
| --- | --- | --- | --- |
| Product requirement | Route and feature scope | 先定义用户行为和边界，才能决定哪些页面和功能属于 capstone。 | 会无限扩展需求，无法完成可验证切片。 |
| DTO adapter | Feature rendering | 外部数据必须先转换成内部模型。 | UI 会依赖不稳定 transport shape。 |
| Reducer/business rule | Checkout/cart behavior | 复杂交互要有可测试的状态转移。 | 边界分散导致测试只能点 UI 猜行为。 |
| Evidence artifact | Portfolio claim | 作品集陈述必须有代码、测试和文档证据支撑。 | 会把未验证功能写成已完成能力。 |
...
2. parser 创建 criteria，adapter 生成 product view models；
...
- 每个 capstone 页面是否对应明确用户任务，而不是展示组件堆叠？
- DTO、state、form、permission 和 observability 的 owner 是否清楚？
- 作品集文案是否能被测试、ADR、release checklist 或代码路径证明？

### Migration checks

- 把单文件 demo 迁移为 capstone 时，先切出 feature boundary 和 shared boundary。
- 把业务状态迁移到 reducer 或明确 owner，再补行为测试。
- 把“我实现了”改成“证据在哪里”，用 docs 和 tests 支撑。

### Production risk signals

- feature 页面能打开但业务规则不可测，说明状态边界不够清楚。
- 权限或 release flag 散落在组件里，说明 shared policy 边界缺失。
- 作品集说明和代码行为不一致，必须降低声明或补证据。

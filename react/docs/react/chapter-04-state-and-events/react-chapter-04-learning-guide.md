...
| --- | --- | --- | --- |
| 本章练习总入口 | `src/learning/react/chapter-04-state-and-events/chapter-04-practice-root.tsx` | 真实练习文件 | 8 |
| 本章共享样式 | `src/learning/react/chapter-04-state-and-events/chapter-04-practice.css` | 真实练习文件 | 8 |
| event handler callback | `src/learning/react/chapter-04-state-and-events/01-event-handler-callback/event-handler-callback.tsx` | 真实练习文件 | 9.1 |
| 传递与调用 handler | `src/learning/react/chapter-04-state-and-events/02-pass-vs-call-handler/pass-vs-call-handler.tsx` | 真实练习文件 | 9.2 |
| render 期间调用 handler 的错误 | `Snippet: called handler during render` | 概念 snippet | 9.2 |
| event object、传播和默认行为 | `src/learning/react/chapter-04-state-and-events/03-event-object-and-default-behavior/event-object-and-default-behavior.tsx` | 真实练习文件 | 9.3 |
| useState 组件记忆 | `src/learning/react/chapter-04-state-and-events/04-use-state-memory/use-state-memory.tsx` | 真实练习文件 | 9.4 |
| local variable 与 state | `src/learning/react/chapter-04-state-and-events/05-local-variable-vs-state/local-variable-vs-state.tsx` | 真实练习文件 | 9.5 |
| state setter 与当前 render | `src/learning/react/chapter-04-state-and-events/06-state-setter-current-render/state-setter-current-render.tsx` | 真实练习文件 | 9.6 |
| state snapshot 与 closure | `src/learning/react/chapter-04-state-and-events/07-state-snapshot/state-snapshot.tsx` | 真实练习文件 | 9.7 |
| batched replacement updates | `src/learning/react/chapter-04-state-and-events/08-batched-updates/batched-updates.tsx` | 真实练习文件 | 9.8 |
| functional updater queue | `src/learning/react/chapter-04-state-and-events/09-functional-update/functional-update.tsx` | 真实练习文件 | 9.9 |
| object state replacement | `src/learning/react/chapter-04-state-and-events/10-object-state-update/object-state-update.tsx` | 真实练习文件 | 9.10 |
| direct object mutation 错误 | `Snippet: direct object state mutation` | 概念 snippet | 9.10 |
| array state replacement | `src/learning/react/chapter-04-state-and-events/11-array-state-update/array-state-update.tsx` | 真实练习文件 | 9.11 |
| direct array mutation 错误 | `Snippet: direct array state mutation` | 概念 snippet | 9.11 |
| typed event handler | `src/learning/react/chapter-04-state-and-events/12-typed-event-handler/typed-event-handler.tsx` | 真实练习文件 | 9.12 |
| mini project 类型 | `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-types.ts` | 最终小项目文件 | 12 |
| mini project seed data | `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-seed-data.ts` | 最终小项目文件 | 12 |
| quantity controls | `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-quantity-controls.tsx` | 最终小项目文件 | 12 |
| cart item row | `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-item-row.tsx` | 最终小项目文件 | 12 |
| derived cart summary | `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-summary.tsx` | 最终小项目文件 | 12 |
| mini project root | `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-quantity-mini-project.tsx` | 最终小项目文件 | 12 |
| mini project styles | `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-quantity-mini-project.css` | 最终小项目文件 | 12 |
| Vite app 挂载 adapter | `src/App.tsx` | 已更新入口文件 | 12 |

## 0. 文件定位

本文件是当前 React + TypeScript + Vite 学习路径的第四章。第一章划清 React、TypeScript、Vite 和 browser DOM 的职责；第二章建立 JSX 与 component 模型；第三章把 props 解释为 parent-to-child 输入；第四章第一次系统回答“用户操作后，页面为什么会变”。

本章只覆盖 React 交互的第一层机制：event handler、`useState`、render snapshot、update queue、batching、functional update、对象与数组的 immutable update，以及 TypeScript event type。它不引入 effect、context、reducer、router、data fetching、TanStack Query、React Hook Form、backend API、Prisma 或 SellerHub 实现。

当前项目证据：

- `package.json` 声明 React `^19.2.0`、TypeScript `~5.9.3` 和 Vite `^7.2.4`。
- `tsconfig.app.json` 启用 `strict`、`noEmit` 和 `jsx: "react-jsx"`。
- `index.html` 加载 `/src/sudoku/main.tsx`；该文件创建 React root 并导入根级 `src/App.tsx`。
- 根级 `src/App.tsx` 现在只作为 thin adapter，挂载 `Chapter04PracticeRoot`。
- Chapter 04 的真实练习全部位于 `src/learning/react/chapter-04-state-and-events/`，没有修改 `src/sudoku/`。
...
1. 先理解 callback：React 何时调用 handler。
2. 再理解 browser event：event object 能控制什么，不能控制什么。
3. 引入 `useState`：为什么需要 React-managed memory。
4. 对比 local variable：排除普通变量模型。
5. 学习 setter、snapshot、batching：建立 update queue 模型。
6. 学习 functional update：基于 queued previous state 计算。
7. 将同一模型应用到 object 和 array。
8. 最后补 TypeScript compile-time boundary，并用 cart 集成全部机制。

这个顺序先建立 JavaScript callback 和 browser event 边界，再进入 React state runtime，最后才讨论 TypeScript。这样不会把 type annotation 误认为运行时行为。
...
| File | Role | Status |
| --- | --- | --- |
| `docs/react/chapter-04-state-and-events/react-chapter-04-learning-guide.md` | 第四章完整学习指导。 | 已创建并保留。 |
| `src/App.tsx` | 挂载 Chapter 04 practice root 的 thin adapter。 | 已更新并保留。 |
| `README.md` | 更新 Chapter 04 路线状态。 | 已更新并保留。 |

本章普通真实练习文件：

| File | Role | Status |
| --- | --- | --- |
| `src/learning/react/chapter-04-state-and-events/chapter-04-practice-root.tsx` | 汇总所有真实练习与 mini project。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/chapter-04-practice.css` | 本章共享样式。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/01-event-handler-callback/event-handler-callback.tsx` | Callback timing。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/02-pass-vs-call-handler/pass-vs-call-handler.tsx` | Function value vs call。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/03-event-object-and-default-behavior/event-object-and-default-behavior.tsx` | Event object、propagation、default。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/04-use-state-memory/use-state-memory.tsx` | State memory。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/05-local-variable-vs-state/local-variable-vs-state.tsx` | Local binding vs state。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/06-state-setter-current-render/state-setter-current-render.tsx` | Setter 与 current render。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/07-state-snapshot/state-snapshot.tsx` | Snapshot 与 closure。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/08-batched-updates/batched-updates.tsx` | Batched replacement updates。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/09-functional-update/functional-update.tsx` | Updater queue。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/10-object-state-update/object-state-update.tsx` | Object replacement。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/11-array-state-update/array-state-update.tsx` | Array replacement。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/12-typed-event-handler/typed-event-handler.tsx` | Event 与 state types。 | 已创建并保留。 |

最终小项目真实文件：

| File | Role | Status |
| --- | --- | --- |
| `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-types.ts` | Product 和 CartItem types。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-seed-data.ts` | Typed local seed data。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-quantity-controls.tsx` | Quantity controls 与 disabled boundaries。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-item-row.tsx` | Cart row composition。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-summary.tsx` | Derived units 和 total。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-quantity-mini-project.tsx` | Cart state owner 与 updates。 | 已创建并保留。 |
| `src/learning/react/chapter-04-state-and-events/cart-quantity-mini-project/cart-quantity-mini-project.css` | Mini project styles。 | 已创建并保留。 |

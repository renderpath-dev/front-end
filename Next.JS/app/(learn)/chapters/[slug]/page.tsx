  ...
  return (
    <div className="grid gap-12 xl:grid-cols-[minmax(0,1fr)_16rem]">
      <article className="min-w-0">
        <AnimatedReveal>
          <header className="overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-sm shadow-slate-950/5 sm:p-8">
        ...
        <div className="space-y-16 py-12">
            ...
            <section className="scroll-mt-24" id="objectives">
              ...
              <ul className="mt-6 grid gap-3">
                {chapterContent.objectives.map((objective) => (
                  <li
                    className="flex gap-3 rounded-xl border border-border bg-card p-4 text-sm leading-6 text-muted shadow-sm shadow-slate-950/5"
                    key={objective}
                  >
            ...
            <section className="scroll-mt-24" id="mechanism-chain">
              ...
              <ol className="mt-8 grid gap-4">
                {chapterContent.mechanismChain.map((item) => (
                  <li
                    className="grid gap-4 rounded-xl border border-border bg-card p-5 shadow-sm shadow-slate-950/5 sm:grid-cols-[3rem_minmax(0,1fr)_auto]"
                    key={item.step}
                  >
          ...
          <AnimatedReveal>
            <section className="scroll-mt-24" id="boundary-audit">
              ...
              <div className="mt-6 overflow-x-auto rounded-xl border border-border bg-card shadow-sm shadow-slate-950/5">
          ...
          <AnimatedReveal>
            <section className="scroll-mt-24" id="code-examples">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-accent">
                Code boundary lab
              </p>
              <h2 className="mt-3 text-2xl font-bold tracking-tight text-foreground">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-muted">
                Each example declares its boundary and runtime before the code
                window, so syntax highlighting supports the mechanism instead
                of replacing it.
              </p>
              <div className="mt-6 grid gap-6">
                {chapterContent.codeExamples.map((example) => (
                  <article
                    className="rounded-2xl border border-border bg-card p-4 shadow-sm shadow-slate-950/5 sm:p-5"
                    key={example.title}
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <BoundaryBadge kind={example.boundary} />
                    ...
                    <div className="mt-4">
                      <CodeWindow
                        code={example.code}
                        description={example.learningPoint}
          ...
          <AnimatedReveal>
            <section className="scroll-mt-24" id="experience-boundaries">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-accent">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-muted">
                Visual polish is useful only when the execution boundary stays
                clear. These notes explain why each enhancement lives where it
                does.
              </p>
              <AnimatedList className="mt-6 grid gap-4 md:grid-cols-2">
                {chapterContent.experienceBoundaries.map((note) => (
                  <article
                    className="h-full rounded-xl border border-border bg-card p-5 shadow-sm shadow-slate-950/5"
                    key={note.title}
                  >
                    <div className="flex justify-start">
                      <BoundaryBadge kind={note.boundary} />
                    </div>
            ...
            <section className="scroll-mt-24" id="experiments">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                {chapterContent.experiments.map((experiment) => (
                  <article
                    className="rounded-xl border border-border bg-card p-5 shadow-sm shadow-slate-950/5"
